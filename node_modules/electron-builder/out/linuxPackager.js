...
`);
    ...
    buildDeb(options, outDir, appOutDir, arch) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const args = ["-s", "dir", "-t", target, "--architecture", archName, "--rpm-os", "linux", "--name", this.metadata.name, "--force", "--after-install", scripts[0], "--after-remove", scripts[1], "--description", `${ options.synopsis || "" }\n ${ this.debOptions.description }`, "--maintainer", author, "--vendor", options.vendor || author, "--version", this.metadata.version, "--package", destination, "--deb-compression", options.compression || (this.devMetadata.build.compression === "store" ? "gz" : "xz"), "--url", projectUrl];
