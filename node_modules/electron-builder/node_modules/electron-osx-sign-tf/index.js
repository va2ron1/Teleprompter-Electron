...
function signApplication (opts, callback) {
  ...
  var codeSignBaseArgs = [
    '--sign', opts.identity,
    '-fv'
  ]
  if (opts.keychain) {
    codeSignBaseArgs.push('--keychain', opts.keychain)
  }
  ...
  if (opts.entitlements) {
    ...
    childPaths.forEach(function (filePath) {
      operations.push(function (cb) {
        child.execFile('codesign', codeSignBaseArgs.concat('--entitlements', opts['entitlements-inherit'], filePath), function (err, stdout, stderr) {
          if (err) return cb(err)
          cb()
        })
    ...
    operations.push(function (cb) {
      child.execFile('codesign', codeSignBaseArgs.concat('--entitlements', opts.entitlements, opts.app), function (err, stdout, stderr) {
        if (err) return cb(err)
        cb()
      })
    ...
    childPaths.forEach(function (filePath) {
      operations.push(function (cb) {
        child.execFile('codesign', codeSignBaseArgs.concat(filePath), function (err, stdout, stderr) {
          if (err) return cb(err)
          cb()
        })
        debuglog('Signing... ' + filePath)
    ...
    operations.push(function (cb) {
      child.execFile('codesign', codeSignBaseArgs.concat(opts.app), function (err, stdout, stderr) {
        if (err) return cb(err)
        cb()
      })
      debuglog('Signing... ' + opts.app)
